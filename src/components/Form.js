  ...
  if (props.tasks.length) {
    clearBtn = (
      <div className="col-md-2 mt-4">
        <button type="button" className="btn btn-block btn-danger" onClick={props.clearLocalStorage} > <FontAwesomeIcon icon={faTrashAlt} /> ALL </button>
        {props.tasks.filter((e) => e.done === true).length ? <button type="button" className="btn btn-block  btn-warning" onClick={props.clearCompleted} > <FontAwesomeIcon icon={faTrashAlt} /> Completed </button> : null}
      </div>
